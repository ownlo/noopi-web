...
test('keeps the role private and exposes server-approved card targets', () => {
  ...
  assert.equal(breakOptions.length, 3)
...
test('repair card targets only a player and repairs the first broken tool', () => {
  ...
  const breakCards = playing.cardOptions.filter(option => option.actionType === 'BREAK_TOOL')
  const pickaxe = breakCards.find(option => option.targets[0].toolType === 'PICKAXE')
  const lantern = breakCards.find(option => option.targets[0].toolType === 'LANTERN')
  assert.ok(pickaxe && lantern)
  mock.play(1, { cardId: pickaxe.cardId, actionType: 'BREAK_TOOL', targetPlayerId: 2, toolType: 'PICKAXE' })
  mock.play(1, { cardId: lantern.cardId, actionType: 'BREAK_TOOL', targetPlayerId: 2, toolType: 'LANTERN' })
  ...
  playing = mock.snapshot(1)
  assert.deepEqual(playing.players.find(player => player.playerId === 2).brokenTools, ['LANTERN'])
})
