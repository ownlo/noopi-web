...
export function YutGameGuide({ game, onClose, onAction, actionLabel = '시작하기', showCloseButton = true, actionVariant = 'primary' }: { game: GameCatalog['games'][number]; onClose: () => void; onAction: () => void; actionLabel?: string; showCloseButton?: boolean; actionVariant?: 'primary' | 'secondary' }) {
  ...
  return createPortal(<div className="dialogBackdrop gameGuideBackdrop" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) onClose() }}><section className="gameGuideDialog yutGameGuideDialog" role="dialog" aria-modal="true" aria-labelledby="yut-guide-title" aria-describedby="yut-guide-summary"><header className="gameGuideHeader"><div><p>NOOPI GAME GUIDE</p><h2 id="yut-guide-title">{game.name}</h2></div>{showCloseButton && <button type="button" autoFocus onClick={onClose} aria-label="게임 방법 닫기">×</button>}</header><p className="gameGuideSummary" id="yut-guide-summary">윷을 던져 말을 움직여요. 개인전은 모두 완주할 때까지 순위를 겨뤄요!</p><div className="gameGuideMeta"><span>👥 {game.minPlayers}–{game.maxPlayers}명</span><span>🏁 개인전 · 2:2 팀전</span></div><ol className="gameGuideSteps yutGuideSteps"><li><div className="yutGuideIcon">🎲</div><div><small>STEP 1</small><h3>윷을 던져요</h3><p>내 차례가 오면 윷을 던져요. 윷이나 모가 나오면 한 번 더 던져요. 낙은 그 던지기만 무효이고, 앞서 얻은 이동권은 그대로 사용해요.</p></div></li><li><div className="yutGuideIcon">🐾</div><div><small>STEP 2</small><h3>말을 움직여요</h3><p>나온 칸 수만큼 움직일 말을 골라요. 갈림길에서는 지름길도 갈 수 있어요.</p></div></li><li><div className="yutGuideIcon">🏆</div><div><small>STEP 3</small><h3>끝까지 순위를 정해요</h3><p>개인전은 말 4개를 완주한 순서대로 관전하고, 모두 끝나면 순위표를 확인해요. 팀전은 먼저 완주한 팀이 승리해요.</p></div></li></ol><div className="gameGuideTip yutGuideTip"><b>누피의 팁</b><span>말을 함께 업으면 빨리 갈 수 있지만, 잡히면 모두 출발점으로 돌아가니 조심하세요!</span></div><Button className={`gameGuideStart ${actionVariant === 'secondary' ? 'secondary' : ''}`} autoFocus={!showCloseButton} onClick={onAction}>{actionLabel}</Button></section></div>, document.body)
}
