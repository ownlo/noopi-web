...
const names = { NAK: '낙', BACK_DO: '빽도', DO: '도', GAE: '개', GEOL: '걸', YUT: '윷', MO: '모' }
...
const flatFaceIndexes: Record<YutResultCode, number[]> = {
  NAK: [], BACK_DO: [0], DO: [1], GAE: [0, 1], GEOL: [0, 1, 2], YUT: [0, 1, 2, 3], MO: [],
}
export function YutThrowScene({ result, active, animationId, power = 0.5 }: { result?: YutResultCode; active: boolean; animationId: number; power?: number }) {
  ...
  return <div className={`yutThrowScene ${active ? 'isTossing' : ''} ${result === 'NAK' ? 'isNak' : ''}`} style={style} aria-hidden="true">
    <div className="yutThrowMat" />
    <div className="yutFlyingSet" key={animationId}>
      {[0, 1, 2, 3].map(index => <div className="yutFlyingLane" key={index} style={{ '--i': index, '--angle': `${[-28, 21, -12, 38][index]}deg`, '--land-x': `${[-66, -21, 24, 65][index]}px`, '--land-y': `${[3, -13, 9, -3][index]}px`, '--nak-x': `${[-310, -155, 160, 315][index]}px`, '--nak-shadow-x': `${[-120, -60, 64, 126][index]}px` } as CSSProperties}>
