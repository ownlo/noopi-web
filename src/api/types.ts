...
export type RoomState = { room: { roomId: number; roomCode: string; status: 'WAITING' | 'ACTIVE' | 'CLOSED'; hostPlayerId: number }; me: Player; players: Player[]; gameSession: null | { gameSessionId: number; gameType: 'LIAR'; status: 'READY' | 'PLAYING' | 'FINISHED' | 'CANCELLED'; gameState: LiarGameState } }
export type GameCatalog = { games: { gameType: 'LIAR'; name: string; minPlayers: number; maxPlayers: number; enabled: boolean }[] }
