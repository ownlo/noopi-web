...
function Setup({ categories, category, setCategory, pending, onCreate }: { categories:{code:string;name:string;virtual:boolean}[];category:string;setCategory:(v:string)=>void;pending:boolean;onCreate:()=>void }) { return <><div className="gameIcon">😈</div><p className="eyebrow">라이어 게임</p><h1>카테고리를<br />골라주세요</h1><div className="categoryGrid">{categories.map(c=><button key={c.code} className={category===c.code?'selected':''} onClick={()=>setCategory(c.code)}><b>{c.name}</b>{category===c.code&&<i>✓</i>}</button>)}</div><Card className="rule"><span>👥 참가자 4명</span><span>😈 라이어는 항상 1명</span></Card><Button disabled={!category||pending} onClick={onCreate}>{pending?'준비 중...':'이 카테고리로 준비하기'}</Button></> }
