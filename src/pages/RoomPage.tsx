...
import { api } from '../api'
import type { BlindGameState, GameCatalog, GameState, GameType, RealtimeEvent, LiarGameState, MafiaGameState, MafiaJudgmentChoice, MafiaNightActionType, YutGameState, YutMode, YutTeamId, YutThrowResult } from '../api/types'
...
export function RoomPage() {
  ...
  const [pigRollEvent, setPigRollEvent] = useState<RealtimeEvent | null>(null)
  const games = useQuery({ queryKey: ['games'], queryFn: api.getGames, enabled: Number.isFinite(roomId) })
  ...
  const mePlayerId = stateQuery.data?.me.playerId
  ...
  useEffect(() => api.subscribe(roomId, event => {
    ...
    if (event.type === 'PLAYER_LEFT' && event.payload.playerId === mePlayerId) {
      localStorage.removeItem('noopi.lastRoomId')
      allowNavigationRef.current = true
      navigate('/', { replace: true })
      return
    ...
    if (event.type === 'PIG_ROLL_RESOLVED') setPigRollEvent(previous => previous?.eventId === event.eventId ? previous : event)
    void sync()
  }, isUp => { setConnected(isUp); if (isUp) void sync() }), [mePlayerId, navigate, roomId, showReturnedLobby, sync])
  useEffect(() => {
    if (!isRoomUnavailable(stateQuery.error)) return
    localStorage.removeItem('noopi.lastRoomId')
    allowNavigationRef.current = true
    navigate('/', { replace: true })
  ...
  const kickMutation = useMutation({ mutationFn: (playerId: number) => api.kickPlayer(roomId, playerId), onSuccess: () => { setNotice(''); void sync() }, onError: () => { setNotice('참가자를 내보내지 못했어요. 상태를 다시 확인해주세요.'); void sync() } })
...
function GameSelect({ catalog, loading, onSelect }: { catalog: GameCatalog; loading: boolean; onSelect: (game: GameCatalog['games'][number]) => void }) {
  const [guideGame, setGuideGame] = useState<GameCatalog['games'][number] | null>(null)
  const [selectedCategory, setSelectedCategory] = useState('ALL')
  const categories = [...catalog.catalogCategories].sort((left, right) => left.order - right.order)
  const games = catalog.games.filter(game => game.enabled && (selectedCategory === 'ALL' || game.catalogCategoryCodes.includes(selectedCategory)))
  useEffect(() => {
    if (!guideGame) return
  ...
  useEffect(() => {
    if (selectedCategory !== 'ALL' && !catalog.catalogCategories.some(category => category.code === selectedCategory)) setSelectedCategory('ALL')
  }, [catalog.catalogCategories, selectedCategory])

  return <>
    <h1>무슨 게임을 할까요?</h1>
    <p className="sub">오늘 분위기에 딱 맞는 게임을 골라보세요.</p>
    <div className="gameCategoryFilters" aria-label="게임 카테고리">
      <button type="button" aria-pressed={selectedCategory === 'ALL'} onClick={() => setSelectedCategory('ALL')}>전체</button>
      {categories.map(category => <button key={category.code} type="button" aria-pressed={selectedCategory === category.code} onClick={() => setSelectedCategory(category.code)}>{category.name}</button>)}
    </div>
    {games.map(game => <article className={`gameChoice${game.gameType === 'PIG' ? ' gameChoicePig' : ''}${game.gameType === 'TOOTH' ? ' gameChoiceTooth' : ''}`} key={game.gameType}>
