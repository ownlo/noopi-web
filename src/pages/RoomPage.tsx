...
export function RoomPage() {
  ...
  return <Page><header className="roomHeader"><Brand /><span className="roomPill" aria-label={`내 닉네임 ${state.me.nickname}`}><span className={connected ? 'live' : 'offline'} />{state.me.nickname}</span></header>{!connected && <div className="network">연결이 불안정해요. 다시 연결하고 있습니다...</div>}{notice && <div className="toast" role="status">{notice}</div>}<PlayerGenderProvider players={state.players}><div className="content">{game && !choosingNextGame ? <Game key={state.gameSession?.gameSessionId} game={game} state={state} pending={mutation.isPending} act={act} /> : screen === 'GAMES' && state.me.host ? <GameSelect games={games.data?.games ?? []} onSelect={selected => { const reason = getGameUnavailableReason(selected, state.players.length); setNotice(reason ?? ''); if (!reason) setScreen('SETUP') }} /> : screen === 'SETUP' && state.me.host ? <Setup unavailableReason={setupUnavailableReason} playerCount={state.players.length} categories={categories.data?.categories ?? []} category={category} setCategory={setCategory} pending={mutation.isPending} onCreate={() => mutation.mutate({type:'CREATE',payload:category})} /> : <RoomLobby state={state} onSelect={() => setScreen('GAMES')} />}</div></PlayerGenderProvider></Page>
}
...
function GameSelect({ games, onSelect }: { games: GameCatalog['games']; onSelect: (game: GameCatalog['games'][number]) => void }) {
  return <>
    <p className="eyebrow">GAME SELECT</p>
    <h1>무슨 게임을 할까요?</h1>
...
function Setup({ unavailableReason, playerCount, categories, category, setCategory, pending, onCreate }: { unavailableReason:string|null;playerCount:number;categories:{code:string;name:string;virtual:boolean}[];category:string;setCategory:(v:string)=>void;pending:boolean;onCreate:()=>void }) { return <><div className="gameIcon">😈</div><p className="eyebrow">라이어 게임</p><h1>카테고리를<br />골라주세요</h1><div className="categoryGrid">{categories.map(c=><button key={c.code} className={category===c.code?'selected':''} onClick={()=>setCategory(c.code)}><b>{c.name}</b>{category===c.code&&<i>✓</i>}</button>)}</div><Card className="rule"><span>👥 참가자 {playerCount}명</span><span>😈 라이어 1명</span></Card><p className="hint" role="status">{unavailableReason}</p><Button disabled={!category||pending||unavailableReason !== null} onClick={onCreate}>{pending?'준비 중...':'이 카테고리로 준비하기'}</Button></> }
