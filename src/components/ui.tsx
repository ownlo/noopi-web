...
import maleCharacter from '../assets/characters/male-3d.png'
import femaleCharacter from '../assets/characters/female-3d.png'
...
export function Avatar({ name, gender }: { name?: string; gender?: 'MALE' | 'FEMALE' }) { const players = useContext(PlayerGenderContext); const resolvedGender = gender ?? players.find(player => player.nickname === name)?.gender; const source = resolvedGender === 'FEMALE' ? femaleCharacter : maleCharacter; return <span className={`avatar ${resolvedGender === 'FEMALE' ? 'pink' : ''}`} aria-hidden><img src={source} alt="" /></span> }
