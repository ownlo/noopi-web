...
지원 게임 목록은 카탈로그 카테고리와 각 게임의 복수 카테고리 소속을 함께
제공한다. 이 카테고리는 방장이 게임 목록을 탐색하기 위한 표시 메타데이터이며
게임 규칙이나 GameSession 설정이 아니다. 따라서 선택한 카탈로그 카테고리를
GameSession에 저장하거나 게임 시작 요청의 `config`로 전달하지 않는다.

Frontend는 서버가 제공한 카테고리와 소속 관계를 기준으로 게임 목록을
필터링한다. `전체`는 모든 활성 게임을 보여주는 Client 전용 가상 필터다.
...
예를 들어 라이어 게임의 역할과 투표, 블라인드 게임의 Player별 제시어
배정과 정답 판정, 윷놀이의 팀·턴·말 이동과 승리 판정, 누피 콱!의 꽝 이빨·
턴·당첨 판정, 언더마인의 라운드·역할·카드·길·금 지급 판정은 각각 해당 게임
규칙에서 관리한다.

언더마인의 3개 라운드는 하나의 `UNDERMINE` GameSession 내부 단계다. 다음
라운드를 시작할 때 GameSession을 새로 만들지 않으며, 3라운드의 금 지급과
최종 순위 확정이 끝났을 때 해당 GameSession을 `FINISHED`로 변경한다.
