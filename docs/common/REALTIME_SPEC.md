...
언더마인의 역할, 손패, 지도 확인 결과, 금 카드와 게임 종료 전 누적 금점수는
개인화 정보다. Room broadcast에는 공개된 카드 행동, 보드 변경, 공개 목적지,
턴과 라운드 전환처럼 모두가 볼 수 있는 정보만 포함한다. 버리기 이벤트에는
버린 카드의 식별자나 종류를 포함하지 않는다. Client는 이벤트 수신 후 개인화된
`/state`를 다시 조회하며 카드 배치 가능 여부, 장비 대상, 라운드 승패 또는
금점수를 이벤트만으로 계산하지 않는다. 전체 종료는 공통 `GAME_FINISHED`를
사용한다.
...
이빨 선택, 언더마인 카드 행동과 금 선택 등 중요한 행동은 서버에서
중복 실행을 방지해야 한다.
