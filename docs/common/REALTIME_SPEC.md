...
`YUT_THROW_RESOLVED`의 `NAK`는 이동권 없이 즉시 턴이 종료된 공개 결과이며,
Client는 `/state`의 `lastThrow`와 새 턴을 다시 조회해 낙 연출과 화면을 동기화한다.
...
게임 시작, 역할 확인, 투표, 라이어 최종 추측, 블라인드 정답 제출 등 중요한 행동은 서버에서
중복 실행을 방지해야 한다.
